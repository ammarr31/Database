...
FROM customers c  JOIN employees e ON c.salesperson_id = e.employee_id;

SELECT o.order_id, od.product_id 
FROM orders o JOIN order_details od ON o.order_id = od.order_id;

SELECT s.name AS student_name, i.name AS instructor_name 
FROM students s JOIN instructors i ON s.instructor_id = i.instructor_id;
...
SELECT c.course_date AS course_date, e.exam_date AS exam_date 
FROM courses c JOIN exams e ON c.course_id = e.course_id;
...
SELECT e.employee_name, d.location AS department_location 
FROM employees e JOIN departments d ON e.department_id = d.department_id;
